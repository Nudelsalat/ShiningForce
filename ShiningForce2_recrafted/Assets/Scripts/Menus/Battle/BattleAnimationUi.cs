...
namespace Assets.Scripts.Menus.Battle {
    public class BattleAnimationUi : MonoBehaviour {
        ...
        public void DoAttackAnimation(bool isDodge, bool isKill, bool isCrit, string spellPath, EnumMagicType? magicAttackType) {
            ...
            if (!isDodge && isAttack) {
                ...
                StartCoroutine(DelayHitAnimations(isKill, isCrit, _spawnedTarget, length - 0.15f));
            }
            StartCoroutine(DelayedUpdate(0.5f));
        }

        public void DoCounterAnimation(bool isDodge, bool isKill, bool isCrit) {
            ...
            if (!isDodge) {
                ...
                var length = clip != null ? clip.length : 0.5f;
                StartCoroutine(DelayHitAnimations(isKill, isCrit, _spawnedAttacker, length - 0.15f));
            }
            StartCoroutine(DelayedUpdate(0.5f));
        }
        ...
        IEnumerator FlashAndWiggle(bool isKill, bool isCrit, GameObject target) {
            if (isCrit) {
                _audioManager.PlaySFX(Constants.SfxCritHit);
            } 
        ...
        IEnumerator DelayHitAnimations(bool isKill, bool isCrit, GameObject target, float seconds) {
            yield return new WaitForSeconds(seconds);
            StartCoroutine(FlashAndWiggle(isKill, isCrit, target));
